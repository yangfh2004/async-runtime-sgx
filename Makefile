...
SGX_MODE ?= HW
SGX_ARCH ?= x64
TEACLAVE_SDK ?= $(CURDIR)/rust-sgx-sdk

_DUMMY_ARG := $(shell ./rust-sgx-sdk.bash)
...
CUSTOM_EDL_PATH := $(TEACLAVE_SDK)/sgx_edl/edl
...
Rust_Target_Path := $(TEACLAVE_SDK)/rustlib

Rust_Build_Std := $(Rust_Build_Flags) -Zbuild-std=core,alloc
Rust_Std_Features := --features backtrace,capi,net,thread,untrusted_time,untrusted_fs,unsupported_process
...
RustEnclave_Out_Path := target/$(Rust_Build_Target)/$(Rust_Build_Out)
RustEnclave_Lib_Name := $(RustEnclave_Out_Path)/libasyncruntime.a
